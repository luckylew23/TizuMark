...
![五种提示框渲染效果（Note / Tip / Important / Warning / Caution）](screenshots/14-callout.png)
...
**字体自定义** — 编辑器 / 预览 / 代码块字体可分别选择，**系统已安装字体全量列出、支持搜索**（等线、微软雅黑、苹方、思源黑体…装了就能选）；更可以**导入你自己下载的字体文件**（ttf / otf / woff 等），连字体预览都给你准备好了。想用思源黑体、霞鹜文楷还是任何小众字体，装进去就行。

![字体设置](screenshots/07-font.png)
...
加粗、斜体、删除线、链接、图片、水平线、高亮、上标、下标，常用操作一键完成。还有「结构插入」下拉：表格、代码块、引用块、数学公式、Mermaid、目录，模板自动填入；列表、六级标题、五种提示块也都触手可及。记不住 `[]` 还是 `()`？不用记。工具栏可**一键折叠**释放编辑空间；回车换行渲染为 `<br>` 还是保持传统段落，开关可控。
...
- **图片查看器**：预览中点击任意图片弹出专用查看器，支持鼠标拖拽平移、滚轮居中缩放
...
更强大的是**跨文件搜索**：`Ctrl+Shift+F` 可搜索当前所有标签页或指定目录，搜索结果在编辑区与预览区同时高亮导航，一键跳转。写大型项目文档时不必逐个文件翻。
...
不只是单文件编辑。`文件 → 打开文件夹` 把整个目录装进来，侧边栏文件树随时浏览、点击即开。外部程序增删文件后自动刷新，跟 IDE 一样方便，但没有 IDE 的臃肿。

![文件夹工作区 + 侧边栏文件树](screenshots/13-workspace.png)
...
**图片查看器** — 预览中点击任意图片，弹出专用查看器。支持鼠标拖拽平移、滚轮居中缩放，无需离开软件就能细看大图

**任务列表可点击** — 预览中的勾选框不是摆设，直接点击就能勾选/取消勾选，编辑器源码自动同步

**标签页拖拽排序** — 拖动标签自由调整顺序，就跟浏览器一样自然
...
**最近打开文件** — 自动记录最近编辑的文件，一键重新打开，不用每次翻目录

**系统托盘** — 可显示托盘图标，关闭窗口可选退出、最小化到托盘或每次询问，后台挂着随时呼出

**快捷键预设** — 内置 Default / VSCode / Typora / Sublime Text 四种预设方案，一键切换，也可逐条自定义

**安全可靠** — 后端用 Rust 对渲染内容做 HTML 净化、防 XSS，无需担心恶意文档

无需注册账号，无需联网，完全离线可用。代码开源（GPL-3.0）。
...
> 💡 **首次装/升级后，打开 TizuMark 会自动弹出「使用说明」和完整语法演示 `demo.md`**，无需翻阅文档即可快速上手全部功能。也可在 `帮助 → 使用说明` 中随时查看。
